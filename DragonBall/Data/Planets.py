...
from DragonBall.Constants.ScrapPlanet import ScrapPlanet
...
class Planets:
    ...
    @classmethod
    def planet_list(cls, planet_list: list):
        return [Planets(i).planet_information() for i in planet_list]

    @staticmethod
    def list():
        request = requests.get(URL.URL_LIST_PLANETS)
        soup = BeautifulSoup(request.content.decode('utf-8'), "html.parser")
        ref = soup.find('div', {"class": "mw-parser-output"}).find_all('b')
        list_planets = [planet.text for planet in ref]
        list_planets.pop(0)
        return list_planets

    def planet_information(self):
        ...
        section_dict = self.__evaluate_scrapper_section(list_section, list(list_section.keys()), ScrapPlanet)
    ...
    def __obtain_basic_data(self, soup: BeautifulSoup, url: str):
        ...
        description = self.__get_description_fusion(soup)
    ...
    @staticmethod
    def __get_description_fusion(soup: BeautifulSoup):
    ...
    def __evaluate_scrapper_section(self, list_dict, list_search, tag_found: ScrapPlanet):
