...
from DragonBall.Constants.ScrapGames import ScrapGames
...
class VideoGames:
    ...
    @classmethod
    def game_list(cls, game_list: list):
        return [VideoGames(i).game_information() for i in game_list]

    @staticmethod
    def list():
        request = requests.get(URL.URL_LIST_GAMES)
        soup = BeautifulSoup(request.content.decode('utf-8'), "html.parser")
        summary_data = soup.find("div", {"id": "toc"}).text.strip().splitlines()
        return Summary.clean_list_summary(summary_data,[1,-4])
    ...
    def game_information(self):
        ...
        section_dict = self.__evaluate_scrapper_section(list_section, list(list_section.keys()), ScrapGames)
    ...
    def __obtain_basic_data(self, soup: BeautifulSoup, url: str):
        ...
        description = self.__get_description_game(soup)
    ...
    @staticmethod
    def __get_description_game(soup: BeautifulSoup):
    ...
    def __evaluate_scrapper_section(self, list_dict, list_search, tag_found: ScrapGames):
